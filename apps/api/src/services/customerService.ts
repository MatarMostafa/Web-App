...
export const getCustomerOrderByIdService = async (customerId: string, orderId: string) => {
  ...
  const formattedContainers = order.containers.map(container => ({
    ...
    articles: container.articles.map(article => ({
      ...
      // Exclude price information for customers
    }))
  }));
...
export const createCustomerOrderService = async (userId: string, orderData: any) => {
  ...
  // Calculate proper prices for containers based on selected activities
  let processedContainers = orderData.containers || [];
  if (processedContainers.length > 0 && orderData.activities?.length > 0) {
      ...
      processedContainers.map(async (container: any) => {
        ...
        for (const activity of orderData.activities) {
          try {
            ...
            // Fetch activity definition to get articleBasePrice (price per activity type)
            const definition = await prisma.customerActivity.findUnique({
              where: { id: activity.activityId }
            });
            if (definition) {
              piecePrice += Number(definition.articleBasePrice) || 0;
            }
          } catch (error) {
  ...
  const sanitizedOrderData = {
    ...
    pieceQuantity: orderData.pieceQuantity,
...
export const updateCustomerOrderService = async (orderId: string, orderData: any) => {
  // Get the existing order to access customer information
