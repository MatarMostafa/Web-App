...
export const getOrderFiles = async (req: Request, res: Response) => {
  try {
    ...
    const files = await fileService.getFilesByOrderService(orderId);
    res.json(files);
  } catch (error) {
...
export const getAssignmentFiles = async (req: Request, res: Response) => {
  try {
    const { assignmentId } = req.params;
    const files = await fileService.getFilesByAssignmentService(assignmentId);
    res.json(files);
  } catch (error) {
    console.error("Get assignment files error:", error);
    res.status(500).json({
      message: "Failed to get assignment files",
      error: error instanceof Error ? error.message : String(error)
    });
  }
};
...
export const updateFile = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { description, documentType, expiryDate, isPublic } = req.body;

    const updatedFile = await fileService.updateFileService(id, {
      description,
      documentType,
      expiryDate: expiryDate ? new Date(expiryDate) : undefined,
      isPublic
...
// Get expiring files
export const getExpiringFiles = async (req: Request, res: Response) => {
  try {
    const { days } = req.query;
    const daysAhead = days ? parseInt(days as string) : 30;
    
    const files = await fileService.getExpiringFilesService(daysAhead);
    res.json(files);
  } catch (error) {
    console.error("Get expiring files error:", error);
    res.status(500).json({
...
export const verifyFile = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const verifiedFile = await fileService.verifyFileService(id, req.user?.id || '');
