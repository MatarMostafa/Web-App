...
export const PRICING_TYPES = ['HOURLY', 'PER_PIECE', 'PER_CARTON', 'PER_ARTICLE'] as const;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger asChild>
        <Button>{t("activities.addActivity")}</Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t("activities.form.addTitle")}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 mt-2 border rounded-md p-3">
              {PRICING_TYPES.map(pt => (
                <div key={pt} className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor={`pt-add-${pt}`} className="text-sm font-normal cursor-pointer">
              ...
              {priceRanges.map((range, index) => (
                <div key={index} className="grid grid-cols-5 gap-2 mb-2 items-end">
                  ...
                  <div>
                    ...
                    <Input type="date" value={range.validFrom} onChange={(e) => updatePriceRange(index, 'validFrom', e.target.value)} required />
                  </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? t("activities.form.creating") : t("activities.addActivity")}
            </Button>
          </div>
        </form>
      </DialogContent>
